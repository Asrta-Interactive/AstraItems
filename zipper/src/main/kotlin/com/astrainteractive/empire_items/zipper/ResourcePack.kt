...
import com.astrainteractive.empire_items.zipper.data.*
import com.astrainteractive.empire_items.zipper.data.Override
...
class ResourcePack(private val empireItemsAPI: EmpireItemsAPI) {
    ...
    data class Sound(val sounds: List<String>) {
        constructor(sound: YmlSound) : this(sound.sounds)
    }
    ...
    private fun resolveParent(item: YmlItem): String {
        val name = item.material
        return when {
    ...
    private fun resolvePotion(item: YmlItem) =
        if (item.material.contains("potion", ignoreCase = true)) "minecraft:item/potion" else null

    private fun createAutoGeneratedModel(item: YmlItem) {
        ...
        if (item.material == Material.BOW.name.uppercase()) {
            for (i in 0..2) {
    ...
    fun createMinecraftModel(item: YmlItem) {
        if (item.customModelData == null || item.customModelData == 0)
            return
        if (item.texturePath == null && item.modelPath == null)
            return
        ...
        val file = File(getMinecraftItemsModelsPath() + sep + item.material.lowercase() + ".json")
        if (item.material == Material.BOW.name.uppercase()) {
            if (!file.exists())
                saveFileFromResources("pack/base/item/bow.json", getMinecraftItemsModelsPath() + sep + "bow.json")
        } else if (item.material == Material.SHIELD.name.uppercase()) {
            if (!file.exists()) {
        ...
        } else if (item.material == Material.POTION.name.uppercase()) {
            if (!file.exists())
        ...
        if (model == null) {
            model = Model(
                if (item.block == null) resolveParent(item) else "minecraft:block/${item.material.lowercase()}",
                textures = Textures(
                    layer0 = "minecraft:item/${item.material.lowercase()}"
                )
            )
        }
        var predicate =
            Predicate(customModelData = item.customModelData, pulling = if (item.material == Material.BOW.name.uppercase()) 0 else null)
        ...
        if (item.material == Material.SHIELD.name.uppercase()) {
    ...
    companion object {
        ...
        fun generate(empireItemsAPI: EmpireItemsAPI) = ResourcePack(empireItemsAPI)
