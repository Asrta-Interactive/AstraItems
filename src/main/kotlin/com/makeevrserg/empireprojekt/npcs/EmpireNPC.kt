...
class EmpireNPC {
    ...
    lateinit var nmsNpc: EntityPlayer
    ...
    private fun spawnNPC(location: Location, profile: GameProfile) {
        ...
        nmsNpc.listName = IChatBaseComponent.a("")
    ...
    private fun saveLocation(location: Location) {
        EmpirePlugin.empireFiles.npcs.getConfig()!!.set("npcs.${nmsNpc.name}.location", location)
...
//                playerSpawnedList.remove(connection)
    ...
    fun changeSkin(p: Player, playerName: String) {
        hideNPCFromOnlinePlayers()
        setNPCSkin(p, saveConfig = true, playerName, nmsNpc.profile)
        showNPCToOnlinePlayers()
    }

    fun relocateNPC(p: Player) {
        hideNPCFromOnlinePlayers()
        nmsNpc.setLocation(p.location)
