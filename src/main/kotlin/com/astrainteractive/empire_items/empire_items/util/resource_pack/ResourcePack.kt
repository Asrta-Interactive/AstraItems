...
import com.astrainteractive.empire_items.api.YmlSound
...
class ResourcePack {
    ...
    private fun generateFont() {
        ...
        EmpireItemsAPI.fontByID.forEach {
            val p = Providers.Provider.fromAstraFont(it.value)
    ...
    private fun generateSounds() {
        val sounds = EmpireItemsAPI.itemYamlFiles.mapNotNull { it.ymlSounds?.values }.flatten()
    ...
    private fun createAutoGeneratedModel(item: YmlItem) {
        ...
        if (item.material == Material.BOW.name) {
            for (i in 0..2) {
    ...
    fun createMinecraftModel(item: YmlItem) {
        ...
        } else if (item.material == Material.SHIELD.name) {
            if (!file.exists()) {
        ...
        } else if (item.material == Material.POTION.name) {
            if (!file.exists())
        ...
        var predicate =
            Predicate(customModelData = item.customModelData, pulling = if (item.material == Material.BOW.name) 0 else null)
        ...
        if (item.material == Material.SHIELD.name) {
    ...
    private fun generateItems() {
        ...
        val simpleItems = EmpireItemsAPI.itemYamlFilesByID.values.filter { it.block==null }
